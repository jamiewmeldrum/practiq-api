...
public class AdminDocumentControllerCT {
    ...
    @Test
    void postDocumentAcceptsContentLengthAtTheMaximumAndErrorsOneAbove() {
        when(documentRepository.save(any(DocumentEntity.class)))
                .thenAnswer(invocation -> TestReflection.setField(invocation.getArgument(0), "id", 99L));

        Map<String, Object> atTheMaximum = aValidRequestBody();
        atTheMaximum.put("contentLength", DOCUMENT_UPLOAD_MAX_CONTENT_LENGTH);
        ...
        requestBody.put("contentLength", DOCUMENT_UPLOAD_MAX_CONTENT_LENGTH + 1);
        ...
        given().when()
                ...
                .contentType(ContentType.JSON)
                .body("keySet()", containsInAnyOrder("error", "status"))
                .body("error", equalTo("contentLength: must not be greater than " + DOCUMENT_UPLOAD_MAX_CONTENT_LENGTH))
                .body("status", equalTo(413));

        // Still the one save from the accepted request above: one over the maximum wrote nothing.
    ...
    @Test
    void postDocumentAcceptsFilenameAtTheMaximumLengthAndErrorsOneAbove() {
        when(documentRepository.save(any(DocumentEntity.class)))
                .thenAnswer(invocation -> TestReflection.setField(invocation.getArgument(0), "id", 99L));

        Map<String, Object> atTheMaximum = aValidRequestBody();
        atTheMaximum.put("filename", "a".repeat(DOCUMENT_FILENAME_MAX_LENGTH - ".txt".length()) + ".txt");
        ...
        Map<String, Object> oneAbove = aValidRequestBody();
        oneAbove.put("filename", "a".repeat(DOCUMENT_FILENAME_MAX_LENGTH + 1 - ".txt".length()) + ".txt");
        ...
        given().when()
                ...
                .contentType(ContentType.JSON)
                .body("keySet()", containsInAnyOrder("error", "status"))
                .body("error", equalTo("filename: size must be between 0 and " + DOCUMENT_FILENAME_MAX_LENGTH))
                .body("status", equalTo(422));
    ...
    @Test
    void postDocumentAcceptsSourceSpecAtTheMaximumLengthAndErrorsOneAbove() {
        when(documentRepository.save(any(DocumentEntity.class)))
                .thenAnswer(invocation -> TestReflection.setField(invocation.getArgument(0), "id", 99L));

        Map<String, Object> atTheMaximum = aValidRequestBody();
        atTheMaximum.put("sourceSpec", "a".repeat(DOCUMENT_SOURCE_SPEC_MAX_LENGTH));
        ...
        Map<String, Object> oneAbove = aValidRequestBody();
        oneAbove.put("sourceSpec", "a".repeat(DOCUMENT_SOURCE_SPEC_MAX_LENGTH + 1));
        ...
        given().when()
                ...
                .contentType(ContentType.JSON)
                .body("keySet()", containsInAnyOrder("error", "status"))
                .body("error", equalTo("sourceSpec: size must be between 0 and " + DOCUMENT_SOURCE_SPEC_MAX_LENGTH))
                .body("status", equalTo(422));
