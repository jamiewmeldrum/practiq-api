...
// Real web layer through to the real service, accessor, policy, query runner and mappers — only the
...
public class MarkSchemeControllerCT {
    ...
    void getMarkSchemeSerialisesResponse() {
        ...
        MarkSchemeEntity markScheme = new MarkSchemeEntity(questionId, body);
        ...
        // spec is built inside the query runner, so it can only be matched by type.
    ...
    @Test
    void getMarkSchemeReturnsBadRequestIfQuestionIdNotANumber() {
    ...
    @Test
    void getMarkSchemeReturnsEnvelopeForQuestionIdBelowOneAndNeverAsksTheRepositories() {
        // A question id below one can never name a row, so it is refused as a bad value rather than answered
        // 404 as an absent one. Both arms live in one test so the boundary cannot drift into separate cases.
        for (String questionId : List.of("0", "-4002")) {
            given().when()
                    .get(QUESTIONS_PATH + "/" + questionId + "/mark-scheme")
                    .then()
        ...
        // Refused at the web edge, so neither the visibility gate nor the mark-scheme lookup is reached.
