...
public class UnsatisfiedRouteExceptionHandler
        implements ExceptionHandler<UnsatisfiedRouteException, HttpResponse<ErrorResponse>> {
    @Override
    public HttpResponse<ErrorResponse> handle(HttpRequest request, UnsatisfiedRouteException exception) {
        ...
        return HttpResponseFactory.INSTANCE.status(
                BAD_REQUEST, new ErrorResponse(exception.getMessage(), BAD_REQUEST.getCode()));
    }
}
