...
public class UnsatisfiedBodyRouteExceptionHandler
        implements ExceptionHandler<UnsatisfiedBodyRouteException, HttpResponse<ErrorResponse>> {
    @Override
    public HttpResponse<ErrorResponse> handle(HttpRequest request, UnsatisfiedBodyRouteException exception) {
        ...
        return HttpResponseFactory.INSTANCE.status(
                BAD_REQUEST, new ErrorResponse("Request body not specified", BAD_REQUEST.getCode()));
    }
}
